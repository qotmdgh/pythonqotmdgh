...
def get_exchange_rate_data(code, currency_name):
    ...
    total_rate_data_veiw(df, code, currency_name)

def total_rate_data_veiw(df, code, currency_name):
    #원하는 열만 선택
...
#     month_rate_data_veiw(df_total)


# def month_rate_data_veiw(df_total):
#     #월별검색
#     #날짜 열 형변환(문자열 --> 날짜형식)
...
def exchange_main():
    currency_symbols_name={'미국 달러':'USD', 'EU 유로':'EUR', '일본 엔':'JPY'}
